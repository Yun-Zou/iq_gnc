...
enum state { Grounded, TakeOff, Hover, Flight, Search, Follow, Landing };
...
class FlightAlgorithm {
    ...
    public:
        ...
        void set_flight_mode(state mode);
        ...
        std::vector<gnc_api_waypoint> search_grid(geometry_msgs::Point search_origin, std::pair<double, double> size, double spacing) {
          ...
          while (nextWayPoint.y < bounding_box.y) {
            while (nextWayPoint.x < bounding_box.x) {
              nextWayPoint.x = nextWayPoint.x +
                               std::min(2.0, bounding_box.x - nextWayPoint.x);
              waypointList.push_back(nextWayPoint);
            }

            nextWayPoint.y = nextWayPoint.y +
                             std::min(spacing, bounding_box.y - nextWayPoint.y);
            waypointList.push_back(nextWayPoint);

            while (nextWayPoint.x > search_origin.x) {
              nextWayPoint.x = nextWayPoint.x -
                               std::min(2.0, nextWayPoint.x - search_origin.x);
              waypointList.push_back(nextWayPoint);
            }
        ...
        std::vector<gnc_api_waypoint> setCircularWaypoint(geometry_msgs::Point centre, float radius, float alt, float start = M_PI, float rotation = 2 * M_PI) {
          float min_arc = 1;
          ...
          while (current_rotation < (rotation + start)) {
            current_rotation =
                ceil(
                    (current_rotation +
                     std::min(min_angle, rotation + start - current_rotation)) *
                    100) /
                100;
        ...
        std::vector<gnc_api_waypoint> getWayponts() {
            return waypointList;
        };
